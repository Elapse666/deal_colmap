...
class ATEEvaluator(object):
    ...
    def evaluate(self, gt_traj, est_traj, scale):
        ...
        # 用Horn (closed-form)方法将两个轨迹对齐
        rot, trans, trans_error, s = align(gt_xyz, est_xyz, scale)  
        print('  ATE scale: {}'.format(s))     # 将尺度信息打印出来 
        error = np.sqrt(np.dot(trans_error,trans_error) / len(trans_error))  

        # align two trajs  
        est_SEs = pos_quats2SE_matrices(est_traj)   # est_SEs 为 n个4x4矩阵的列表
        T = np.eye(4)       # 对齐所用的T即将用于将轨迹每一步的姿态也做对齐
        T[:3,:3] = rot      # 获取之前的对齐 旋转矩阵
        T[:3,3:] = trans    
        T = np.linalg.inv(T)    # T求逆
        est_traj_aligned = []   #
        for se in est_SEs:
            se[:3,3] = se[:3,3] * s
            se_new = T.dot(se)              # Ti\se 即inv(Ti)*se
            se_new = SE2pos_quat(se_new)    # 矩阵转换四元素
