...
class TartanAirEvaluator:
    ...
    def evaluate_one_trajectory(self, gt_traj, est_traj, scale=False, title=''):
        ...
        传入估计的轨迹和真实的轨迹, 返回一个字典, 包含了ATE, RPE, KITTI的评估结果
        这个函数要用
        """
        # 轨迹数据不相同，报错  shape[0]行的个数、shape[1]列的个数
        ...
        gt_traj = gt_traj.astype(np.float64)    # 用来提升精度，避免后续运算损失
        est_traj = est_traj.astype(np.float64)

        # 传入轨迹，计算ATE分数、以及返还对齐后的轨迹数据
